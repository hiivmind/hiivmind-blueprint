...
Unlike `bp-author-prose-migrate` (which converts existing prose), this skill generates
...
| BLUEPRINT_LIB_VERSION.yaml | `BLUEPRINT_LIB_VERSION.yaml` | Copy from `${CLAUDE_PLUGIN_ROOT}/BLUEPRINT_LIB_VERSION.yaml` |
...
HANDLE_SKILL_NAME(response):
  ...
  # e.g., "bp-author-skill-create" -> "skill-create"
...
DETECT_CONTEXT():
  ...
  version_files = Glob("BLUEPRINT_LIB_VERSION.yaml")
  computed.context.has_version_file = len(version_files) > 0
  ...
  # If version file exists, read lib version
  IF computed.context.has_version_file:
    version_content = Read("BLUEPRINT_LIB_VERSION.yaml")
    computed.lib_version = extract_yaml(version_content, ".lib_version")
    computed.lib_ref = extract_yaml(version_content, ".lib_ref")
  ELSE:
...
| `has_version_file` | `false` | Create BLUEPRINT_LIB_VERSION.yaml in Phase 6 |
...
DETERMINE_ACTIONS():
  computed.actions = {
    ...
    create_version_file:     NOT computed.context.has_version_file,
...
| `{{lib_version}}` | `computed.lib_version` | From BLUEPRINT_LIB_VERSION.yaml or default |
| `{{lib_ref}}` | `computed.lib_ref` | From BLUEPRINT_LIB_VERSION.yaml or default |
...
### Step 6.3: Create BLUEPRINT_LIB_VERSION.yaml

If `BLUEPRINT_LIB_VERSION.yaml` does not exist, copy it from the plugin root:

```pseudocode
CREATE_VERSION_FILE():
  IF NOT computed.actions.create_version_file:
    SKIP "Version file already exists"
    RETURN

  # Copy from the author plugin's version file as a template
  source_path = "${CLAUDE_PLUGIN_ROOT}/BLUEPRINT_LIB_VERSION.yaml"
  source_content = Read(source_path)

  IF source_content IS EMPTY:
    # Fall back to generating a minimal version file
    source_content = """
# External Library Reference Configuration
lib_version: "v3.0.0"
lib_ref: "hiivmind/hiivmind-blueprint-lib@v3.0.0"
lib_raw_url: "https://raw.githubusercontent.com/hiivmind/hiivmind-blueprint-lib/v3.0.0"
schema_version: "2.3"
engine_version: "1.0.0"
"""

  Write("BLUEPRINT_LIB_VERSION.yaml", source_content)
  computed.files_created.append("BLUEPRINT_LIB_VERSION.yaml")

  DISPLAY "Created: BLUEPRINT_LIB_VERSION.yaml"
```
...
VALIDATE_FILES():
  ...
  IF computed.actions.create_version_file:
    expected_files.append("BLUEPRINT_LIB_VERSION.yaml")
...
│       ├── SKILL.md          ← NEW
│       └── workflow.yaml     ← NEW
└── BLUEPRINT_LIB_VERSION.yaml {new/existing}
...
4. **Create gateway** -- Run `bp-author-gateway-create` to set up gateway routing
{/if}
...
- **Library Version Config:** `${CLAUDE_PLUGIN_ROOT}/BLUEPRINT_LIB_VERSION.yaml`
...
- Deep skill analysis: `${CLAUDE_PLUGIN_ROOT}/skills-prose/bp-author-skill-analyze/SKILL.md`
- Prose-to-workflow migration: `${CLAUDE_PLUGIN_ROOT}/skills-prose/bp-author-prose-migrate/SKILL.md`
- Gateway creation: `${CLAUDE_PLUGIN_ROOT}/skills-prose/bp-author-gateway-create/SKILL.md`
- Skill validation: `${CLAUDE_PLUGIN_ROOT}/skills-prose/bp-author-skill-validate/SKILL.md`
