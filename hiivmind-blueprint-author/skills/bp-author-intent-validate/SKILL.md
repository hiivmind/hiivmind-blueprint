---
name: bp-author-intent-validate
description: >
...
```json
{
  "questions": [{
    ...
    "options": [
      {
        "label": "Fix issues",
        "description": "Apply suggested fixes to the intent-mapping.yaml (will invoke bp-author-intent-create)"
      },
      {
        "label": "Re-validate",
...
if user_choice == "Fix issues":
    # Hand off to bp-author-intent-create with computed.validation as context
    # The create skill can use the issue list to guide targeted fixes
    invoke_skill("bp-author-intent-create", context=computed.validation)
...
- **Create intent mapping:** `${CLAUDE_PLUGIN_ROOT}/skills-prose/bp-author-intent-create/SKILL.md`
- **Validate workflow:** `${CLAUDE_PLUGIN_ROOT}/skills-prose/bp-author-skill-validate/SKILL.md`
- **Validate gateway command:** `${CLAUDE_PLUGIN_ROOT}/skills-prose/bp-author-gateway-validate/SKILL.md`
