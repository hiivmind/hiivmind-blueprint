...
Deep structural analysis of an existing prose-based SKILL.md to extract phases, conditionals, actions, state variables, user interactions, and complexity. Produces analysis YAML for use by `bp-author-prose-migrate`.
...
The output analysis YAML is the **handoff contract** consumed by `bp-author-prose-migrate`.
...
Assemble the complete analysis from all `computed.*` state into a single structured document. This is the primary output and the handoff contract to `bp-author-prose-migrate`.
...
```json
{
  "questions": [{
    ...
    "multiSelect": false,
    "options": [
      {"label": "Migrate to workflow (Recommended)", "description": "Convert this skill to workflow.yaml using bp-author-prose-migrate"},
...
Based on response:
- **Migrate to workflow**: Hand off `computed.analysis` to `bp-author-prose-migrate`. The analysis YAML is the input contract -- `bp-author-prose-migrate` expects the exact schema produced in Step 4.1.
...
computed.allowed_tools     computed.state_variables        computed.conversion_recommendations
                           ...
                           computed.analysis_warnings                           bp-author-prose-migrate
```
...
- **Migrate prose skill to workflow:** `bp-author-prose-migrate`
- **Discover installed plugins:** `bp-author-plugin-discover`
- **Analyze workflow-based skill:** `bp-author-skill-analyze`
