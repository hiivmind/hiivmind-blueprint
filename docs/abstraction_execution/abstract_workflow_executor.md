...
We've externalized type definitions to `hiivmind/hiivmind-blueprint-types` (43 consequences, 27 preconditions). The workflow schema v2.1 supports external type references via `definitions` block.
...
1. Loads type definitions from external sources
...
Copy from hiivmind-blueprint-types as fallback for offline/airgapped scenarios:
- `index.yaml`
- `consequences/`
- `preconditions/`
