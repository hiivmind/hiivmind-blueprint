...
description: >
  ...
  "normalize loader", "thin out SKILL.md", "reduce loader size", "upgrade gateway",
  "upgrade command", "migrate gateway command", or needs to update existing skill or gateway
...
4. Migrating loader files to use remote execution references
5. Removing obsolete `.hiivmind/blueprint/engine.md` files
6. Normalizing "fat" loaders to match template standards

---

## Target Types

| Target | Directory | Template | Fat Threshold | Extra Preserved Sections |
|--------|-----------|----------|---------------|--------------------------|
| `skills` | `skills/` | `thin-loader.md.template` (~60 lines) | 120 lines | - |
...
Migrate loader files from local engine.md references to remote execution URLs.
...
Normalize "fat" loaders to match template standards. Fat loaders contain execution pseudocode and framework documentation that belongs in blueprint-lib.
...
- `/hiivmind-blueprint-ops upgrade --normalize-only` - Only normalize fat loaders
- `/hiivmind-blueprint-ops upgrade --skip-normalize` - Skip loader normalization

---

## Phase 1: Discover Workflows and Determine Target
...
### Step 1.2: Detect Available Targets

Scan for both skill and gateway workflows:

```
# Find skill workflows
skill_workflows = []
for workflow in Glob("skills/**/workflow.yaml"):
  skill_md = dirname(workflow) + "/SKILL.md"
  if file_exists(skill_md):
    skill_workflows.append({
      path: workflow,
      loader_path: skill_md,
      directory: dirname(workflow),
      type: "skill"
    })

# Find gateway workflows (require intent-mapping.yaml sibling)
gateway_workflows = []
for workflow in Glob("commands/**/workflow.yaml"):
  dir = dirname(workflow)
  intent_mapping = dir + "/intent-mapping.yaml"
  if file_exists(intent_mapping):
    # Find the gateway .md file (named after directory)
    gateway_name = basename(dir)
    gateway_md = dir + "/" + gateway_name + ".md"
    gateway_workflows.append({
      path: workflow,
      loader_path: gateway_md,
      intent_mapping_path: intent_mapping,
      directory: dir,
      type: "gateway"
    })

has_skills = len(skill_workflows) > 0
has_gateways = len(gateway_workflows) > 0
```

### Step 1.3: Determine Target Type

Based on command arguments and detected workflows:

```
function determine_target(args, has_skills, has_gateways):
  # Explicit target from arguments
  if args.target == "skills":
    if not has_skills:
      error("No skill workflows found in skills/")
    return "skills"

  if args.target == "gateway":
    if not has_gateways:
      error("No gateway workflows found in commands/")
    return "gateway"

  # Auto-detection
  if has_skills AND has_gateways:
    # Ask user which to upgrade
    return "ask_user"
  elif has_skills:
    return "skills"
  elif has_gateways:
    ...
    error("No workflows found in skills/ or commands/")
```

### Step 1.4: Ask User if Both Types Found
...
{
  "questions": [{
    "question": "Found both skill and gateway workflows. Which would you like to upgrade?",
    "header": "Target",
    "multiSelect": false,
    "options": [
      {"label": "Skills", "description": "Upgrade {skill_count} skill workflows in skills/"},
      {"label": "Gateways", "description": "Upgrade {gateway_count} gateway workflows in commands/"},
      {"label": "Both", "description": "Upgrade all workflows (skills first, then gateways)"},
...
Store selection and set `computed.target_type`.

### Step 1.5: Set Target-Specific Configuration

Based on determined target type:

```
if target_type == "skills":
  computed.workflows = skill_workflows
  computed.fat_threshold = 120
  computed.template = "thin-loader.md.template"
  computed.target_lines = 60
  computed.preserved_sections = ["frontmatter", "title", "workflow_graph", "example_usage", "related_skills"]
  computed.required_sections = ["## Execution Reference"]
  computed.extra_verifications = []

elif target_type == "gateway":
  computed.workflows = gateway_workflows
  computed.fat_threshold = 200
  computed.template = "gateway-command.md.template"
  computed.target_lines = 160
  computed.preserved_sections = ["frontmatter", "title", "workflow_graph", "example_usage", "related_skills", "skill_tables", "help_commands"]
  computed.required_sections = ["## Usage", "## Execution Reference", "## Related Skills"]
  computed.extra_verifications = ["intent_mapping_unchanged"]

elif target_type == "both":
  # Process skills first, then gateways
  computed.process_order = ["skills", "gateway"]
```

### Step 1.6: Select Specific Workflows

If multiple workflows of the selected type found:
...
```json
{
  "questions": [{
    ...
    "multiSelect": false,
    "options": [
      {"label": "All {type}s", "description": "Upgrade all {count} {type} workflows"},
...
Scan loader files for local engine.md references:
...
# Get loaders based on target type
if target_type == "skills":
  loaders = Glob("skills/**/SKILL.md")
elif target_type == "gateway":
  loaders = Glob("commands/**/*.md")
  # Filter to only gateway loaders (with workflow.yaml + intent-mapping.yaml siblings)
  loaders = filter(loaders, has_gateway_siblings)

for loader in loaders:
  content = Read(loader)
  for pattern in legacy_patterns:
    if pattern in content:
      needs_migration.append(loader)
```
...
```
workflow = Read("{directory}/workflow.yaml")
definitions_source = workflow.definitions.source
# e.g., "hiivmind/hiivmind-blueprint-lib@v2.1.0"
...
**Found {count} loader files with local engine.md references.**
...
{
  "questions": [{
    "question": "Migrate loader files to remote execution references?",
    "header": "Migrate",
    "multiSelect": false,
    "options": [
      {"label": "Apply", "description": "Update loader files with remote URLs"},
...
If migration requested, for each loader:
...
Edit(loader, old_section, new_section)
```

### Step 5.5: Remove Obsolete Engine File

If `.hiivmind/blueprint/engine.md` exists and all loader files have been migrated:
...
2. Add them to the top of loader as a "Plugin Context" section
3. Or create a CONTEXT.md file in the directory

---

## Phase 6: Normalize Fat Loaders

Normalize loaders that contain execution pseudocode and framework documentation that belongs in blueprint-lib, not in the loader.

### Step 6.1: Detect Fat Loaders

For each loader with sibling workflow.yaml (and intent-mapping.yaml for gateways):

```
loaders = []
THRESHOLD = computed.fat_threshold  # 120 for skills, 200 for gateways

# Find loaders based on target type
if target_type == "skills":
  for loader in Glob("skills/**/SKILL.md"):
    dir = dirname(loader)
    if file_exists(dir + "/workflow.yaml"):
      loaders.append({path: loader})

elif target_type == "gateway":
  for loader in Glob("commands/**/*.md"):
    dir = dirname(loader)
...
for loader in loaders:
  ...
  # Gateway-specific
  has_extended_3vl = target_type == "gateway" AND matches(content, /## 3VL Intent Detection/) AND section_lines > 20

  is_fat = (line_count > THRESHOLD) AND
           (has_pseudocode OR has_framework_docs OR
            has_algorithm OR has_execution_loop OR has_extended_3vl)
...
After normalization, loaders will match `{computed.template}` (~{computed.target_lines} lines).
```
...
```
function extract_preserved_sections(loader_path, target_type):
  ...
  # Gateway-specific preserved sections
...
# e.g., "hiivmind/hiivmind-blueprint-lib@v2.1.0" → "v2.1.0"
if workflow.definitions?.source:
  lib_version = workflow.definitions.source.split("@")[1]
else:
  lib_version = "v2.1.0"  # Default fallback
```
...
**Target:** ~{computed.target_lines} lines ({computed.template})
...
```json
{
  "questions": [{
    ...
    "multiSelect": false,
    "options": [
      {"label": "Apply", "description": "Normalize to template (~{computed.target_lines} lines)"},
...
template = Read("${CLAUDE_PLUGIN_ROOT}/templates/" + computed.template)
...
# Gateway-specific section insertion
if target_type == "gateway":
...
if new_line_count > computed.fat_threshold:
  warn("Normalized loader still exceeds threshold ({new_line_count} > {computed.fat_threshold})")

# Verify required sections present
for section in computed.required_sections:
...
# Gateway-specific: verify intent-mapping.yaml unchanged
if target_type == "gateway":
...
Check that all loader files now use remote URLs:
...
### Execution Reference Migration
{if refs_migrated}
- {count} loader files updated to use remote execution URLs
...
{/for}
{else}
- No fat loaders found (all within template size limits)
{/if}
...
{if loader_updated}
- `{loader_path}` (execution references migrated)
{/if}
{if loaders_normalized}
{for each normalized_loader}
...
### Next Steps
1. Test the {skill|gateway} to verify behavior unchanged
...
1. **Local engine.md removed**
   ...
   Loader files no longer reference a local `.hiivmind/blueprint/engine.md`.
...
- Analyze skill: `/hiivmind-blueprint-author analyze`
- Convert skill: `/hiivmind-blueprint-author convert`
- Generate files: `/hiivmind-blueprint-author generate`
- Discover skills: `/hiivmind-blueprint-author discover`
