...
description: >
  ...
  or after running bp-prose-analyze.
...
analysis output from `bp-prose-analyze` and produces two files:
...
```
bp-plugin-discover       (inventory)
        |
bp-prose-analyze         (deep analysis)
        |
bp-prose-migrate  <---   THIS SKILL (transformation)
        |
test / iterate                  (manual verification)
```

**Key state handoff:** This skill expects `computed.analysis` to be populated by
`bp-prose-analyze`. If that state is missing, the skill will offer to run
...
object is produced by `bp-prose-analyze` and must include:
...
```json
{
  "questions": [{
    ...
    "options": [
      {
        "label": "Analyze first",
        "description": "Run bp-prose-analyze on a skill, then return here to migrate"
      },
      {
...
HANDLE_MISSING_ANALYSIS(response):
  SWITCH response:
    CASE "Analyze first":
      DISPLAY "Invoke bp-prose-analyze first, then re-invoke this skill."
      DISPLAY ""
      DISPLAY "  Skill(skill: \"bp-prose-analyze\", args: \"<path-to-skill>\")"
      DISPLAY ""
...
### Step 6.2: Load Blueprint Config

Read the centralized version configuration for template substitution:

```pseudocode
LOAD_LIB_VERSION():
  lib_config_path = ".hiivmind/blueprint/config.yaml"

  IF file_exists(lib_config_path):
    lib_config = Read(lib_config_path)
    computed.lib_version = lib_config.lib_version       # e.g., "v3.x.x"
    computed.lib_ref = lib_config.lib_ref               # e.g., "hiivmind/hiivmind-blueprint-lib@v3.x.x"
    computed.lib_raw_url = lib_config.lib_raw_url       # raw GitHub URL base
    computed.schema_version = lib_config.schema_version # e.g., "2.3"
  ELSE:
    # Config file is required - no hardcoded fallback
    DISPLAY "ERROR: Blueprint config not found at .hiivmind/blueprint/config.yaml"
    DISPLAY "Run blueprint setup to initialize the config."
    EXIT
```
...
WRITE_WORKFLOW():
  ...
  # Add header comment
  header = "# Generated by bp-prose-migrate\n"
...
- `definitions.source` references the lib_ref from .hiivmind/blueprint/config.yaml
...
```
computed.analysis (from bp-prose-analyze)
    |
    v
Phase 1: Validate
    |
...
- Skill discovery and inventory: `${CLAUDE_PLUGIN_ROOT}/skills-prose/bp-plugin-discover/SKILL.md`
