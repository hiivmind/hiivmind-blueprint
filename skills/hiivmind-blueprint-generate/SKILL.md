...
if computed.workflow.definitions:
  ...
  # Default to hiivmind-blueprint-types
  definitions_source = "hiivmind/hiivmind-blueprint-types@v1"
```
...
### Step 3.2: Create types.lock

Generate the lock file with version pinning:
...
engine:
  ...
  source: "hiivmind/hiivmind-blueprint@{blueprint_version}"
...
Write to: `{plugin_root}/.hiivmind/blueprint/types.lock`

### Step 3.3: Create Backup (if requested)
...
- `.hiivmind/blueprint/types.lock` (version pinning)
...
| `.hiivmind/blueprint/engine.md` | Workflow execution engine |
| `.hiivmind/blueprint/types.lock` | Version pinning for types and engine |
