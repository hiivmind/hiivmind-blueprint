...
`bp-plugin-discover` but runs non-interactively against the current plugin root.
...
LIB_VERSION_ALIGNMENT():
  ...
  version_file = Read(".hiivmind/blueprint/config.yaml")
...
GENERATE_RECOMMENDATIONS():
  ...
  IF computed.version_consistency.deprecated_count > 0:
    computed.recommendations.append({
      priority: "high",
      category: "consistency",
      message: f"Upgrade {computed.version_consistency.deprecated_count} deprecated type usage(s) to {computed.lib_version} equivalents.",
  ...
  IF NOT computed.version_consistency.lib_version_consistent:
    computed.recommendations.append({
      priority: "high",
      category: "consistency",
      message: "Align all workflows to the same lib version defined in .hiivmind/blueprint/config.yaml.",
      source: "version-consistency"
    })
...
HANDLE_ACTION_RESPONSE(response):
  SWITCH response:
    CASE "Batch validate all":
      ...
      DISPLAY "  Skill(skill: 'bp-plugin-batch', args: 'validate')"
    ...
    CASE "Batch upgrade":
      ...
      DISPLAY "  Skill(skill: 'bp-skill-upgrade', args: 'batch')"
    ...
    CASE "Fix top issue":
      ...
      IF top_rec.category == "consistency" AND "lib version" in top_rec.message:
        ...
        DISPLAY "  Skill(skill: 'bp-skill-upgrade', args: '{affected_workflow_path}')"
      ELIF top_rec.category == "completeness" AND "gateway" in top_rec.message:
        DISPLAY "To fix, invoke:"
        DISPLAY "  Skill(skill: 'bp-gateway-create')"
      ELSE:
