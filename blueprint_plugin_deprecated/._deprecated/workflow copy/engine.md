...
definitions:
  source: hiivmind/hiivmind-blueprint-lib@v2.0.0  # GitHub shorthand
...
|--------|---------|-------------|
| GitHub shorthand | `owner/repo@v2.0.0` | Fetches from raw GitHub URLs |
...
Branches based on a precondition evaluation. With `audit` enabled, provides comprehensive validation with error collection (replaces deprecated `validation_gate`).
...
node_name:
  ...
  branches:
    on_true: present_manifest_option    # Route when condition is true
    on_false: ask_source_type           # Route when condition is false
```

**With audit mode (replaces validation_gate):**
...
check_environment:
  ...
  condition:
    type: all_of                     # Or any_of, xor_of, none_of
    conditions:
      - type: file_exists
        path: "config.yaml"
      - type: tool_available
  ...
  audit:
    enabled: true
    output: computed.audit_results   # Optional: custom output path
    messages:
...
| `description` | No | Human-readable purpose |
| `condition` | Yes | Single precondition object (often a composite like `all_of`, `any_of`, `xor_of`) |
| `audit.enabled` | No | Enable audit mode (default: false) |
| `audit.output` | No | State path for results (default: `computed.audit_results`) |
| `audit.messages` | No | Error messages keyed by precondition type |
| `branches.on_true` | Yes | Node/ending when true |
| `branches.on_false` | Yes | Node/ending when false |

**Audit output structure:**

When `audit.enabled: true`, all conditions are evaluated (no short-circuit) and results are written to state:

```yaml
computed.audit_results:
  passed: false           # Overall result
  total: 2
  passed_count: 1
  failed_count: 1
  results:
    - index: 0
      condition: { type: file_exists, path: "config.yaml" }
      passed: true
    - index: 1
      condition: { type: tool_available, tool: git }
      passed: false
...
#### 4. Validation Gate Node (DEPRECATED)

> **⚠️ DEPRECATED in v2.0.0** - Use `conditional` node with `audit.enabled: true` instead.
> See [Conditional Node](#2-conditional-node) for the recommended approach.

Runs multiple preconditions; all must pass to proceed.

```yaml
# DEPRECATED - prefer conditional with audit
node_name:
...
**Migration to conditional:**

```yaml
# New equivalent using conditional + audit
node_name:
  type: conditional
  condition:
    type: all_of
  ...
  audit:
    enabled: true
    messages:
      file_exists: "Config file missing"
      tool_available: "Git is not installed"
  branches:
...
**Key differences:**
- Results stored in `computed.audit_results` (not `computed.validation_errors`)
- Can use any composite logic (`any_of`, `xor_of`, `none_of`) not just `all_of`
- Consistent routing fields (`on_true`/`on_false` instead of `on_valid`/`on_invalid`)
...
- `workflow`: Fetches from remote repository, executes as sub-workflow
...
4. Build TypeRegistry with consequence and precondition definitions
5. Validate all types used in workflow exist in registry
...
FUNCTION execute_conditional_node(node, types, state):
    IF node.audit AND node.audit.enabled:
        ...
        # Extract nested conditions from composite precondition
        conditions = get_nested_conditions(node.condition)

        FOR index, condition IN enumerate(conditions):
            result = evaluate_precondition(condition, types, state)
        ...
        IF audit_results.passed:
            target = resolve_routing_target(node.branches.on_true, state)
        ELSE:
            target = resolve_routing_target(node.branches.on_false, state)
    ELSE:
...
FUNCTION get_nested_conditions(condition):
    # For composite preconditions (all_of, any_of, xor_of, none_of),
...
**Semantics:**
- **Normal mode:** Pure evaluation, no side effects, short-circuits on composites
- **Audit mode:** Evaluates ALL conditions, writes results to `state.computed.audit_results`
- Always succeeds (routes to one branch)
- Branch targets support variable interpolation
- Audit mode replaces deprecated `validation_gate` functionality
...
### Validation Gate Node (DEPRECATED)

> **⚠️ DEPRECATED** - Use conditional node with `audit.enabled: true` instead.

```
FUNCTION execute_validation_gate_node(node, types, state):
    # NOTE: This function is deprecated. New code should use
    # execute_conditional_node with audit.enabled instead.
...
- **DEPRECATED:** Use conditional with audit for more flexible validation
...
[WORKFLOW]   → branches.on_false: ask_source_type
```
